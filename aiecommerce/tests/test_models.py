...
def test_product_master_str():
    ...
    master_product = ProductMasterFactory(sku="MASTER-SKU-456", description="A master product for testing")
...
def test_product_master_price_precision():
    ...
    master_product = ProductMasterFactory(price_distributor=price)
    master_product.refresh_from_db()
    assert master_product.price_distributor == price
    assert master_product.price_distributor.as_tuple().exponent == -2
